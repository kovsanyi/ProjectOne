...
namespace ProjectOne
{
    ...
    {
        public const string RootUrlApp = "/App";
        public const string RootUrlClose = "/Close";
        public const string RootUrlDesktop = "/Desktop";

        private readonly ConcurrentDictionary<string, PoUITaskbar> _sessionTaskbar;
        private readonly ConcurrentDictionary<string, ConcurrentDictionary<string, PoUIPage>> _sessionPages;

        private PoSessionContainer()
        {
            _sessionTaskbar = new ConcurrentDictionary<string, PoUITaskbar>();
            _sessionPages = new ConcurrentDictionary<string, ConcurrentDictionary<string, PoUIPage>>();
        }

        public void ProcessRequest(PoHttpContext context, PoHttpSession session)
        {
            var pageKey = CreatePageKey(context.Request);
            if (RedirectToDesktop_IfCloseRequest_AndDoDispose(context, session)) return;
        ...
        private void ResponseRequest(PoHttpContext context, PoHttpSession session, string pageKey)
        {
            ...
            if (context.Request.QueryString.ContainsKey("ClientAction".ToLowerInvariant()))
            {
        ...
        private bool CreateSession_IfNotExists(PoHttpContext context, string sessionId, string pageKey)
        {
            ...
            pagesByUrl = new ConcurrentDictionary<string, PoUIPage>();
